namespace briefCore.Tesseract
{
    ...
    using global::Tesseract.Internal;
    using Interop;
    ...
    public unsafe sealed class Pix : DisposableBase
    {
        ...
        public Pix BinarizeOtsuAdaptiveThreshold(int sx, int sy, int smoothx, int smoothy, float scorefract)
        {
            ...
            IntPtr ppixth, ppixd;
            int result = LeptonicaApi.Native.pixOtsuAdaptiveThreshold(handle, sx, sy, smoothx, smoothy, scorefract, out ppixth, out ppixd);
        ...
        public Pix BinarizeSauvola(int whsize, float factor, bool addborder)
        {
            ...
            int result = LeptonicaApi.Native.pixSauvolaBinarize(handle, whsize, factor, addborder ? 1 : 0, out ppixm, out ppixsd, out ppixth, out ppixd);
        ...
        public Pix BinarizeSauvolaTiled(int whsize, float factor, int nx, int ny)
        {
            ...
            IntPtr ppixth, ppixd;
            int result = LeptonicaApi.Native.pixSauvolaBinarizeTiled(handle, whsize, factor, nx, ny, out ppixth, out ppixd);
        ...
        {
            float pAngle, pConf;
            var resultPixHandle = LeptonicaApi.Native.pixDeskewGeneral(handle, sweep.Reduction, sweep.Range, sweep.Delta, redSearch, thresh, out pAngle, out pConf);
        ...
        public Pix Rotate(float angle, RotationMethod method = RotationMethod.AreaMap, RotationFill fillColor = RotationFill.White, int? width = null, int? height = null)
        {
            ...
            if (Math.Abs(rotations - Math.Floor(rotations)) < VerySmallAngle) {
                ...
                // handle general case
                resultHandle = LeptonicaApi.Native.pixRotate(handle, angle, method, fillColor, width.Value, height.Value);
            }
